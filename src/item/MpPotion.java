...
public class MpPotion extends Item implements Runnable{

    private BufferedImage potion;
    private  final AudioPlayer audio;


    public MpPotion(int x, int y, Player player) {
        super(x, y, player);
    ...
    public boolean collision() {
        if(this.getBounds().intersects(player.getBounds())){
            audio.play();
            if(player.MP + (15+(player.level*2)) > player.MAX_MP){
                player.MP = player.MAX_MP;
            }else{
                player.MP += 15+(player.level*2);
            }
            return true;
        }return false;
    }
